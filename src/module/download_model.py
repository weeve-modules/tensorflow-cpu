    ...
    if PARAMS["MODEL_ZIP_DOWNLOAD_URL"]:
        try:
            ...
            if resp.status_code == 200:
                log.info("Saving the zip model inside Docker container ...")
                ...
                with zipfile.ZipFile("tf_model.zip", "r") as zip_ref:
                    zip_ref.extractall()
                    extracted_folders = [
                        x for x in os.listdir() if x not in file_list_before_unzipping
                    ]
                    if len(extracted_folders) > 0:
                        PARAMS["EXTRACTED_MODEL_PATH"] = extracted_folders[0]
                    else:
                        log.error(
                            "Could not extract the model from zipped file. Probably empty archive."
                        )
                ...
                log.error(
                    f"Could not download the model. {resp.status_code} - {resp.reason}"
                )
